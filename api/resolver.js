...
module.exports = function(req, res) {
    req = req.query.text.trim();
    //console.log(req);
    var thing = JSON.parse(req);
    var term = thing.title.trim();
    
  handleIdString(thing, req, res);
...
function handleIdString(id, req, res) {
  request({
    ...
    headers: {'User-Agent': 'AngelaMixMax'},
    gzip: true,
    json: true,
    timeout: 15 * 1000
    ...
    var html = '<a href="' + id.url + '">' + id.title + '</a>';
    res.json({
      body: html
